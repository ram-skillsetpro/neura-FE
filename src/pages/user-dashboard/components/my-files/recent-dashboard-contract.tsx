...
const RecentDashboardContract: React.FC = () => {
  ...
  console.log(allRecentFiles);
  const [paginationData, setPaginationData] = useState({
    totalItems: (allRecentFiles?.totct ?? 0) as number,
    currentPage: 1,
    itemsPerPage: allRecentFiles.perpg,
  });
  ...
  const getCurrentPageNumber = (newPage: number) => {
    if (newPage >= 1 && newPage <= totalPages && paginationData.currentPage !== newPage) {
      ...
      dispatch(getRecentDashboardFilesList({ pgn: newPage }));
    }
  };
  ...
  useEffect(() => {
    setPaginationData({
      ...
      totalItems: (allRecentFiles?.totct ?? 0) as number,
      itemsPerPage: allRecentFiles.perpg,
      currentPage: allRecentFiles.pgn,
    });
  }, [allRecentFiles]);
  ...
  const handleCloseShareModal = async () => {
    await dispatch(getRecentDashboardFilesList({ dataReset: true }));
    setShareModal(false);
...
export default RecentDashboardContract;
