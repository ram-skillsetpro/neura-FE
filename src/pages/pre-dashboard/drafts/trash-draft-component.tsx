...
const TrashDraftFileView: React.FC = () => {
  ...
  const trashDraftList = useAppSelector((state) => state.drafts?.trashDraftList); // useAppSelector((state) => state.dashboard.trashDraftList);

  const [activeFileOption, setActiveFileOption] = useState(-1);
  const dispatch = useAppDispatch();
  const [rows, setRows] = useState<Array<any>>([]);
  const trashDraftResponse = useAppSelector((state) => state.drafts?.trashDraftResponse);
  const isLoading = useAppSelector((state) => state.drafts.isLoading);

  const [paginationData, setPaginationData] = useState({
    totalItems: trashDraftResponse.totct as number,
    currentPage: 1,
    itemsPerPage: trashDraftResponse.perpg,
  });
  ...
  useEffect(() => {
    ...
    dispatch(getDraftTrashListData(payload));

    return () => {
      dispatch(clearDeleteDraftResponse());
    };
  }, []);

  const { itemsPerPage } = paginationData;
  const totalCount = trashDraftResponse.totct as number;
  ...
  const getCurrentPageNumber = async (newPage: number) => {
    if (newPage >= 1 && newPage <= totalPages && paginationData.currentPage !== newPage) {
      ...
      dispatch(setDeletedDraftListPageCount(newPage));
      await dispatch(getDraftTrashListData(payload));
    }
  };
  ...
  useEffect(() => {
    setRows(trashDraftList?.map((data) => dataGenerator(data)));
  }, [trashDraftList, activeFileOption, selectedItems]);
  ...
  const handleRestoreDraft = (data: any) => {
    ...
    dispatch(deleteDraft(draftId, folder, activeStatus));
  };
...
export default TrashDraftFileView;
