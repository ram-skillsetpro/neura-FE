import "vendor/pdfjs-dist/web/pdf_viewer.css";
...
import type { EventBus, PDFViewer } from "vendor/pdfjs-dist/legacy/web/pdf_viewer.mjs";
...
> {
  ...
  async init() {
    ...
    const pdfjs = await import("vendor/pdfjs-dist/web/pdf_viewer.mjs");
  ...
  getRangeTextWithLineBreaks = (range: any) => {
    const container = document.createElement('div');
    container.appendChild(range.cloneContents());

    const textWithLineBreaks = this.getTextWithLineBreaks(container);
    return textWithLineBreaks;
  };

  getTextWithLineBreaks = (node: any) => {
    let text = '';
    node.childNodes.forEach((child: any) => {
      if (child.nodeType === Node.TEXT_NODE) {
        text += child.textContent;
      } else if (child.nodeType === Node.ELEMENT_NODE) {
        if (['BR', 'P', 'DIV'].includes(child.nodeName)) {
          text += '\n';
        }
        text += this.getTextWithLineBreaks(child);
      }
    });
    return text;
  };
  ...
  afterSelection = () => {
    ...
    const content = {
      text: this.getRangeTextWithLineBreaks(range),
    };
